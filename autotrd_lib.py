...
class autotrd(Lam2300):
    ...
    def __run_A3_config(self, key): # PS
        ...
        if re.search('PM_ConfigOpt', key, re.IGNORECASE):  # Calcualte CV mistach numbers
            ...
            # 2025/12 Wayne fix: calculate number_port from PM count if not already set
            if not hasattr(self, 'number_port') or self.number_port is None:
                # Extract max PM number from pmalias_new (e.g., PEMEL7_PM4 → 4)
                import re as re_module
                pm_numbers = []
                for pm in self.pmalias_new:
                    match = re_module.search(r'PM(\d+)', pm)
                    if match:
                        pm_numbers.append(int(match.group(1)))
                self.number_port = max(pm_numbers) if pm_numbers else len(self.pmalias_new)
                logger.info(f'AtmArm: calculated number_port = {self.number_port} from {self.pmalias_new}')
        ...
        elif re.search('GasBox_Config', key, re.IGNORECASE): # 2025/12 Wayne add for no golden PM case
            if self.golden_pm: # 原有邏輯 (計算 mismatch)
                tmp = {'Variable': ['Gas_diff'], self.golden_pm: '' }
                tmp = pd.DataFrame( tmp ) 
                diff = sum( self.df[ self.ATTR_MATCH ] == False )
                for pm in self.pmalias_new:
                    tmp[pm] = diff       
                self.df_A3 = tmp
                #logger.info( f'self.df_A3-{tmp}' )  # PS
            else: # 沒有 golden PM 時,建立空資料或計算其他統計值
                tmp = {'Variable': ['Gas_diff'] }
                tmp = pd.DataFrame( tmp ) 
                for pm in self.pmalias_new:
                    tmp[pm] = 0  # 或其他預設值
                self.df_A3 = tmp
    ...
    def __run_A3_SSHC(self, key): # PS
        ...
        if re.search('^ZTM|Solo_TM', key, re.IGNORECASE): 
            ...
            elif re.search('Z102_|Z312_', key, re.IGNORECASE):     ##2025/11 Darren for MetalM and NXP [GasX_X]  ## 2025/12 Wayne for LA
            ...
            # elif re.search('Z312_', key, re.IGNORECASE):     ##2025/11 Darren for MetalM and NXP [GasX_X]  ## 2025/12 Wayne for LA
            #     # Make selection robust to MultiIndex or non-string column names , needed_cols = tmp.filter(regex=r'^Gas\d+_\d+(?:\.\d+)?$').columns.tolist()
            #     needed_cols = ['Gas7_3','Gas7_95','Gas7_60']
            elif re.search('Z179_', key, re.IGNORECASE):     
                needed_cols = [ 'Gas8_55', 'Gas9_55', 'Gas10_23' ]                  
            elif re.search('ZF180_', key, re.IGNORECASE): # 2025/11 Waume for VYY     
                # needed_cols = [ 'Gas2_9s', 'Gas8_70s', 'Gas7_60s', 'Gas12_5s' ]
                needed_cols = [col for col in tmp.columns if re.match(r'^Gas\d+_\d+s$', str(col))]                  
